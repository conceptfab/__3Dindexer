...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if config["work_directory"] and not os.path.exists(config["work_directory"]):
            raise ConfigError(f"Katalog roboczy nie istnieje: {config['work_directory']}")
        ...
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                if not validate_config(config):
                    logger.warning("Używam domyślnej konfiguracji z powodu błędów walidacji")
...
def get_allowed_extensions() -> List[str]:
    return get_config_value("security.allowed_extensions", [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"])
