...
def copy_preview_if_newer(src_path, dest_path_in_gallery_previews_dir):
    ...
    if not os.path.exists(dest_path_in_gallery_previews_dir) or \
       os.path.getmtime(src_path) > os.path.getmtime(dest_path_in_gallery_previews_dir):
        try:
...
def generate_breadcrumb(relative_path_from_scanned_root, gallery_root_name):
    ...
    for i, component in enumerate(path_components):
        if i < len(path_components) -1 :
            ...
            parts.append({
                "name": component,
                "link": "../" * (depth - 1 - i) + "index.html" # Link to its own index.html
            })
...
def process_single_index_json(index_json_path, scanned_root_path, gallery_output_base_path, template_env, progress_callback=None):
...
def generate_full_gallery(scanned_root_path, gallery_cache_root_dir="."):
    ...
    gallery_output_base_path = os.path.join(gallery_cache_root_dir, sanitized_folder_name)
    ...
    for dirpath, _, filenames in os.walk(scanned_root_path):
        if "index.json" in filenames:
            ...
            generated_html = process_single_index_json(index_json_file, scanned_root_path, gallery_output_base_path, env, print)
            if dirpath == scanned_root_path and generated_html: # This is the root index.html for the gallery
...
if __name__ == '__main__':
    ...
    import subprocess
    subprocess.run([sys.executable, "scanner_logic.py"]) # Run the test in scanner_logic.py
    ...
    if not os.path.exists(os.path.join(test_scan_dir, "index.json")):
        ...
        if not os.path.exists("templates/gallery_template.html"):
            with open("templates/gallery_template.html", "w") as f:
                f.write("<html><body><h1>Test Template for {{ current_folder_display_name }}</h1></body></html>") # Minimal template
