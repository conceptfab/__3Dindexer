...
from PyQt6.QtWebEngineCore import QWebEnginePage  # Dodano QWebEngineSettings
from PyQt6.QtWebEngineCore import QWebEngineSettings
from PyQt6.QtWebEngineWidgets import QWebEngineView
from PyQt6.QtWidgets import (  # Poprawiony import, jeśli był z gallery_generator; QProgressDialog, # Nie jest używany
    QApplication,
    QFileDialog,
    QHBoxLayout,
    QInputDialog,
...
class GalleryWorker(QThread):
    ...
    def __init__(self, scanned_root_path, gallery_cache_root):
        ...
        print(
            f"GalleryWorker initialized for: {self.scanned_root_path}, cache: {self.gallery_cache_root}",
            flush=True,
        )
    ...
    def run(self):
        ...
        try:
            self.emit_progress(
                f"Rozpoczynam generowanie galerii dla: {self.scanned_root_path}"
            )
            ...
            if root_html_path and os.path.exists(root_html_path):
                ...
                self.emit_progress(
                    f"Funkcja generate_full_gallery zwróciła ścieżkę {root_html_path}, ale plik nie istnieje."
                )
                root_html_path = None
            else:
            ...
            self.emit_progress(
                f"Wystąpił krytyczny błąd podczas generowania galerii (GalleryWorker): {e}"
            )
            import traceback
    ...
    def emit_progress(self, message):
        if (
            hasattr(self, "progress_signal") and self.progress_signal
        ):  # Bezpieczniejsze sprawdzenie
            self.progress_signal.emit(message)
        else:
...
class MainWindow(QMainWindow):
    ...
    AI_GALLERY_CACHE_DIR_NAME = "_gallery_cache_ai"
    ...
    def __init__(self):
        ...
        self.GLOBAL_GALLERY_CACHE_ROOT_DIR = os.path.join(
            self.APP_DATA_DIR, self.GALLERY_CACHE_DIR_NAME
        )
        self.GLOBAL_AI_GALLERY_CACHE_ROOT_DIR = os.path.join(
            self.APP_DATA_DIR, self.AI_GALLERY_CACHE_DIR_NAME
        )
        ...
        print(
            f"GLOBAL_GALLERY_CACHE_ROOT_DIR ustawiony na: {self.GLOBAL_GALLERY_CACHE_ROOT_DIR}",
            flush=True,
        )
        ...
        print(
            f"🔍 INIT - current_work_directory: {self.current_work_directory}",
            flush=True,
        )
        self.init_ui()
        ...
        if self.current_work_directory:
            ...
            if self.current_gallery_root_html and os.path.exists(
                ...
                print(
                    f"🔍 INIT - Brak istniejącej galerii dla {self.current_work_directory}, HTML: {self.current_gallery_root_html}",
                    flush=True,
                )
    ...
    def init_ui(self):
        ...
        self.select_folder_button.setStyleSheet(
            "QPushButton { padding: 8px 16px; border-radius: 6px; } QPushButton:hover { background-color: #2d5aa0; }"
        )
        ...
        self.ai_scan_button.setStyleSheet(
            "QPushButton { padding: 8px 16px; border-radius: 6px; background-color: #2d5a50; } QPushButton:hover { background-color: #3d6a60; }"
        )
        ...
        self.ai_gallery_button.setStyleSheet(
            "QPushButton { padding: 8px 16px; border-radius: 6px; background-color: #5a2d5a; } QPushButton:hover { background-color: #6a3d6a; }"
        )
        ...
        self.clear_gallery_cache_button.setStyleSheet(
            "QPushButton { padding: 8px 16px; border-radius: 6px; } QPushButton:hover { background-color: #c62d42; }"
        )
        ...
        self.toggle_mode_button.setStyleSheet(
            "QPushButton { padding: 8px 16px; border-radius: 6px; background-color: #5a5a2d; } QPushButton:hover { background-color: #6a6a3d; }"
        )
        ...
        self.progress_bar.setStyleSheet(
            "QProgressBar { border-radius: 4px; text-align: center; height: 20px; } QProgressBar::chunk { border-radius: 4px; background-color: #3daee9; }"
        )
        main_layout.addWidget(self.progress_bar)

        self.web_view = QWebEngineView()
        self.web_view.setPage(
            CustomWebEnginePage(self.web_view)
        )  # Użyj CustomWebEnginePage
        ...
        self.size_label = QLabel("Rozmiar: 200px")
        self.size_label.setStyleSheet(
            "color: #ffffff; font-weight: 500; padding: 0 10px;"
        )  # Zmieniony tekst
        ...
        self.statusBar.setStyleSheet(
            "QLabel { background-color: rgba(0, 0, 0, 0.7); color: #ffffff; padding: 5px; border-top: 1px solid rgba(255, 255, 255, 0.1); selection-background-color: #3daee9; selection-color: #ffffff; }"
        )
    ...
    def on_webview_url_changed(self, url):
        ...
        if (
            ...
            and local_path.endswith((".html", ".htm"))
        ):
    ...
    def get_current_gallery_index_html(self):
        ...
        if not specific_gallery_output_path:
            return None
    ...
    def update_status_label(self):
        if self.current_work_directory:
            ...
            self.web_view.setHtml(
                "<html><body><p style='text-align:center; padding-top:50px;'>Wybierz folder roboczy.</p></body></html>"
            )
    ...
    def select_work_directory(self):
        ...
        if folder:
            ...
            if config_manager.set_work_directory(self.current_work_directory):
                self.log_message(
                    f"Ustawiono folder roboczy: {self.current_work_directory}"
                )
            else:
                self.log_message(
                    f"Błąd zapisu konfiguracji dla folderu: {self.current_work_directory}"
                )
            self.update_status_label()
            self.current_gallery_root_html = self.get_current_gallery_index_html()
            if self.current_gallery_root_html and os.path.exists(
                ...
                self.log_message(
                    f"Znaleziono istniejącą galerię dla {self.current_work_directory}. Ładuję."
                )
                self.show_gallery_in_app()
            else:
                self.log_message(
                    f"Brak istniejącej galerii dla {self.current_work_directory}. Proponuję przebudowę."
                )
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    self.web_view.setHtml(
                        "<html><body><p style='text-align:center; padding-top:50px;'>Galeria nie zbudowana.</p></body></html>"
                    )
    ...
    def scan_finished(self):
        ...
        reply = QMessageBox.question(
            ...
            "Czy chcesz teraz przebudować galerię HTML?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.Yes,
        )
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def rebuild_gallery(self, auto_show_after_build=True):
        ...
        self.gallery_thread = GalleryWorker(
            self.current_work_directory, self.GLOBAL_GALLERY_CACHE_ROOT_DIR
        )
    ...
    def gallery_generation_finished(self, root_html_path, auto_show=True):
        ...
        if self.current_gallery_root_html:
            ...
            self.log_message(
                "Nie udało się wygenerować galerii lub zwrócony plik nie istnieje.",
                level="ERROR",
            )
            QMessageBox.warning(
                self,
                "Błąd",
                "Nie udało się wygenerować galerii HTML. Sprawdź logi konsoli.",
            )
            self.web_view.setHtml(
                "<html><body><p style='text-align:center; padding-top:50px;'>Błąd generowania galerii.</p></body></html>"
            )
        self.set_buttons_for_processing(False)
        self.update_gallery_buttons_state()
    ...
    def show_gallery_in_app(self):
        ...
        if gallery_index_html_path and os.path.exists(gallery_index_html_path):
            ...
            )  # To wywoła loadStarted, a potem loadFinished
            ...
            self.log_message(
                f"❌ Nie znaleziono pliku galerii: {gallery_index_html_path}",
                level="WARNING",
            )
            self.web_view.setHtml(
                "<html><body><p style='text-align:center; padding-top:50px;'>Plik galerii nie istnieje.</p></body></html>"
            )
    ...
    def on_gallery_loaded(self, ok):
        if self._on_gallery_loaded_connected:  # Rozłącz po pierwszym wywołaniu
            try:
                self.web_view.loadFinished.disconnect(self.on_gallery_loaded)
            except TypeError:
                pass
    ...
    def closeEvent(self, event):
        ...
        if (
            self.ai_scanner_thread and self.ai_scanner_thread.isRunning()
        ):  # Dodano AI thread
            threads_running = True
        ...
        if threads_running:
            ...
            if self.gallery_thread:
                self.gallery_thread.wait(500)
            if self.ai_scanner_thread:
                self.ai_scanner_thread.wait(500)  # Dodano AI thread
        event.accept()
    ...
    def check_for_learning_matches(self):
        try:
            ...
            self.web_view.page().runJavaScript(
                js_code, self.handle_learning_match_result
            )
        except Exception as e:
    ...
    def handle_learning_match_result(self, result):
        ...
        try:
            ...
            if not match_data:
                return
            self.log_message(
                f"Odebrano dopasowanie: {match_data.get('archiveFile')} <=> {match_data.get('imageFile')}"
            )
            self.save_learning_data_generic(match_data, "learning_data.json")
            self.apply_learning_immediately(match_data)
        except json.JSONDecodeError:
            print(
                f"❌ Błąd JSON w handle_learning_match_result: '{result}'", flush=True
            )
        except Exception as e:
            print(f"❌ Błąd handle_learning_match_result: {e}", flush=True)
        ...
        try:
            scanner_logic.start_scanning(
                ...
                lambda msg: self.log_message(
                    f"Scan ({os.path.basename(folder_path)}): {msg}"
                ),
            )
            self.log_message(
                f"Skanowanie {folder_path} zakończone. Przebudowa całej galerii."
            )
            self.rebuild_gallery(auto_show_after_build=True)
        except Exception as e:
            self.log_message(
                f"Błąd rescan_and_rebuild_specific_folder dla {folder_path}: {e}",
                level="ERROR",
            )
    ...
    def check_for_file_operations(self):
        try:
            ...
            """
            self.web_view.page().runJavaScript(
                js_get_localstorage_item + "('deleteFile', 'latestDelete');",
                self.handle_file_deletion_result,
            )
            self.web_view.page().runJavaScript(
                js_get_localstorage_item + "('moveFiles', 'latestMoveFiles');",
                self.handle_move_files_from_js,
            )
            self.web_view.page().runJavaScript(
                js_get_localstorage_item + "('renameFiles', 'latestRenameFiles');",
                self.handle_rename_files_from_js,
            )
            self.web_view.page().runJavaScript(
                js_get_localstorage_item + "('createFolder', 'latestCreateFolder');",
                self.handle_create_folder_from_js,
            )
        except Exception as e:
    ...
    def handle_file_deletion_result(self, result):
        ...
        try:
            ...
            if not delete_data:
                return
            file_path_str = delete_data.get("filePath")
            file_name = delete_data.get("fileName")
            if not file_path_str:
                self.log_message("Brak filePath do usunięcia.", "WARNING")
                return
            normalized_file_path = os.path.normpath(file_path_str)
            if not os.path.exists(normalized_file_path):
                self.log_message(f"Plik {normalized_file_path} nie istnieje.", "ERROR")
                return
            self.log_message(
                f"Odebrano usunięcie: {file_name} ({normalized_file_path})"
            )
            self.delete_file_to_trash(normalized_file_path)
        except json.JSONDecodeError:
            print(f"❌ Błąd JSON handle_file_deletion_result: '{result}'", flush=True)
        except Exception as e:
            print(f"❌ Błąd handle_file_deletion_result: {e}", flush=True)
            QMessageBox.critical(self, "Błąd", f"Błąd obsługi usuwania: {e}")

    def delete_file_to_trash(self, file_path_to_delete):
        try:
            if not os.path.exists(file_path_to_delete):
                self.log_message(
                    f"Plik {file_path_to_delete} już nie istnieje.", "WARNING"
                )
                return
            file_name = os.path.basename(file_path_to_delete)
            send2trash.send2trash(file_path_to_delete)
            self.log_message(
                f"✅ Usunięto do kosza: {file_name} ({file_path_to_delete})"
            )
            folder_affected = os.path.dirname(file_path_to_delete)
            self.rescan_and_rebuild_specific_folder(folder_affected)
        except Exception as e:
            self.log_message(f"Błąd usuwania {file_path_to_delete}: {e}", "ERROR")
            QMessageBox.critical(self, "Błąd", f"Błąd usuwania: {e}")

    def handle_move_files_from_js(self, result):
        if not result:
            return
        try:
            move_data = json.loads(result)
            selected_files_js = move_data.get("files", [])
            if not selected_files_js:
                self.log_message("Brak plików do przeniesienia z JS.", "INFO")
                return
            self.log_message(
                f"Odebrano przeniesienie dla {len(selected_files_js)} elementów (z JS)."
            )
            self.process_move_files_python_logic(selected_files_js)
        except json.JSONDecodeError:
            print(f"❌ Błąd JSON handle_move_files_from_js: '{result}'", flush=True)
        except Exception as e:
            print(f"❌ Błąd handle_move_files_from_js: {e}", flush=True)

    def handle_rename_files_from_js(self, result):
        if not result:
            return
        try:
            rename_data = json.loads(result)
            selected_files_js = rename_data.get("files", [])
            new_base_name_js = rename_data.get("newBaseName")
            if not selected_files_js or not new_base_name_js:
                self.log_message("Niekompletne dane do zmiany nazwy z JS.", "WARNING")
                return
            self.log_message(
                f"Odebrano zmianę nazwy dla {len(selected_files_js)} plików na '{new_base_name_js}' (z JS)."
            )
            self.process_rename_files_python_logic(selected_files_js, new_base_name_js)
        except json.JSONDecodeError:
            print(f"❌ Błąd JSON handle_rename_files_from_js: '{result}'", flush=True)
        except Exception as e:
            print(f"❌ Błąd handle_rename_files_from_js: {e}", flush=True)
    ...
    def handle_create_folder_from_js(self, result):
        ...
        try:
            ...
            parent_folder_js = create_data.get("parentFolder")
            folder_name_js = create_data.get("folderName")
            if not parent_folder_js or not folder_name_js:
                self.log_message(
                    "Niekompletne dane do tworzenia folderu z JS.", "WARNING"
                )
                return
            actual_parent_folder = os.path.normpath(
                parent_folder_js
            )  # Zakładamy, że JS podaje poprawną ścieżkę
            if not os.path.isdir(actual_parent_folder):
                self.log_message(
                    f"Folder nadrzędny '{actual_parent_folder}' z JS nie istnieje.",
                    "ERROR",
                )
                return
            self.log_message(
                f"Odebrano utworzenie folderu '{folder_name_js}' w '{actual_parent_folder}' (z JS)."
            )
            self.process_create_folder_python_logic(
                actual_parent_folder, folder_name_js
            )
        except json.JSONDecodeError:
            print(f"❌ Błąd JSON handle_create_folder_from_js: '{result}'", flush=True)
        except Exception as e:
    ...
    def process_rename_files_python_logic(
        ...
        if not files_to_process_info or not new_base_name:
            QMessageBox.warning(self, "Brak danych", "Brak plików lub nowej nazwy.")
            return
        renamed_count = 0
        errors = []
        folders_affected = set()
        for file_info in files_to_process_info:
            old_path_str = file_info.get("path")
            old_path = os.path.normpath(old_path_str)
            if not old_path_str or not os.path.exists(old_path):
                self.log_message(f"Plik {old_path} nie istnieje.", "WARNING")
                continue
            file_ext = os.path.splitext(old_path)[1]
            new_file_path = os.path.join(
                os.path.dirname(old_path), f"{new_base_name}{file_ext}"
            )
            try:
                if (
                    os.path.exists(new_file_path)
                    and new_file_path.lower() != old_path.lower()
                ):
                    reply = QMessageBox.question(
                        self,
                        "Plik istnieje",
                        f"Plik {os.path.basename(new_file_path)} istnieje. Zastąpić?",
                        QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                    )
                    if reply != QMessageBox.StandardButton.Yes:
                        continue
                if old_path.lower() != new_file_path.lower():
                    ...
                    self.log_message(
                        f"✅ Zmieniono nazwę: {os.path.basename(old_path)} → {os.path.basename(new_file_path)}"
                    )
            except Exception as e:
                errors.append(
        ...
        if renamed_count > 0:
            QMessageBox.information(
                self, "Sukces", f"Zmieniono nazwy dla {renamed_count} plików."
            )
            for folder in folders_affected:
                self.rescan_and_rebuild_specific_folder(folder)
        if errors:
            QMessageBox.warning(
                self, "Błędy", "Błędy podczas zmiany nazw:\n" + "\n".join(errors)
            )

    def process_create_folder_python_logic(
        self, parent_dir_path: str, new_folder_name: str
    ):
        invalid_chars = '<>:"/\\|?*'
        if any(char in new_folder_name for char in invalid_chars):
            QMessageBox.warning(
                self,
                "Błędna nazwa",
                f"Nazwa zawiera niedozwolone znaki: {invalid_chars}",
            )
            return
        new_folder_full_path = os.path.join(parent_dir_path, new_folder_name)
        if os.path.exists(new_folder_full_path):
            QMessageBox.warning(
                self, "Błąd", f"Folder '{new_folder_name}' już istnieje."
            )
            return
        try:
            os.makedirs(new_folder_full_path)
            self.log_message(f"✅ Utworzono folder: {new_folder_full_path}")
            QMessageBox.information(
                self, "Sukces", f"Utworzono folder: {new_folder_name}"
            )
            self.rescan_and_rebuild_specific_folder(parent_dir_path)
        except Exception as e:
            self.log_message(
                f"Nie można utworzyć folderu '{new_folder_full_path}': {e}", "ERROR"
            )
            QMessageBox.critical(self, "Błąd", f"Nie można utworzyć: {e}")
    ...
    def show_move_files_dialog_python(self):
        ...
        if self.web_view.page():
            self.web_view.page().runJavaScript(
                js_code,
                lambda r: self.process_move_files_python_logic(
                    json.loads(r) if r else []
                ),
            )
    ...
    def show_rename_files_dialog_python(self):
        ...
        if self.web_view.page():
            self.web_view.page().runJavaScript(
                js_code, self._handle_rename_for_python_button
            )

    def _handle_rename_for_python_button(self, result_str):
        selected_files = json.loads(result_str) if result_str else []
        if not selected_files:
            QMessageBox.information(self, "Info", "Nie zaznaczono plików.")
            return
        new_base_name, ok = QInputDialog.getText(
            self, "Zmiana nazwy", "Nowa nazwa bazowa (bez rozszerzenia):"
        )
        if ok and new_base_name.strip():
            self.process_rename_files_python_logic(
                selected_files, new_base_name.strip()
            )
        else:
            self.log_message("Anulowano zmianę nazwy.")
    ...
    def show_create_folder_dialog_python(self):
        ...
        folder_name, ok = QInputDialog.getText(
            self,
            "Nowy folder",
            f"Nazwa nowego folderu (w {os.path.basename(parent_for_new_folder)}):",
        )
        if ok and folder_name.strip():
            self.process_create_folder_python_logic(
                parent_for_new_folder, folder_name.strip()
            )
    ...
    def show_delete_empty_dialog_python(self):
        ...
        if deleted_count > 0:
            QMessageBox.information(
                self, "Sukces", f"Usunięto {deleted_count} pustych folderów."
            )
            self.rebuild_gallery(auto_show_after_build=True)
        else:
        ...
        if errors:
            QMessageBox.warning(
                self, "Błędy", "Błędy podczas usuwania:\n" + "\n".join(errors)
            )
    ...
    def load_learning_data(self):
        try:
            ...
            print(f"❌ Błąd wczytywania danych uczenia: {e}", flush=True)
            return []
    ...
    def show_ai_gallery(self):
        ...
        for root, dirs, files in os.walk(self.current_work_directory):
            if "index.json" in files:
                try:
                    with open(
                        os.path.join(root, "index.json"), "r", encoding="utf-8"
                    ) as f:
                        data = json.load(f)
    ...
    def rebuild_ai_gallery(self, auto_show_after_build=True):
        ...
        self.log_message(
            f"Rozpoczynanie przebudowy galerii AI dla: {self.current_work_directory}"
        )
        self.progress_bar.setVisible(True)
        self.progress_bar.setRange(0, 0)
        ...
        self.gallery_thread = GalleryWorker(
            self.current_work_directory, self.GLOBAL_AI_GALLERY_CACHE_ROOT_DIR
        )
        self.gallery_thread.progress_signal.connect(self.log_message)
        self.gallery_thread.finished_signal.connect(
            lambda path: self.ai_gallery_generation_finished(
                path, auto_show_after_build
            )
        )
        self.set_buttons_for_processing(True)
        self.gallery_thread.start()
    ...
    def ai_gallery_generation_finished(self, root_html_path, auto_show=True):
        self.progress_bar.setVisible(False)
        self.current_ai_gallery_root_html = (
            root_html_path
            if root_html_path and os.path.exists(root_html_path)
            else None
        )
        ...
        if self.current_ai_gallery_root_html:
            self.log_message(
                f"Przebudowa galerii AI zakończona. Główny plik: {self.current_ai_gallery_root_html}"
            )
            if auto_show:
            ...
            QMessageBox.warning(
                self,
                "Błąd",
                "Nie udało się wygenerować galerii AI. Sprawdź logi konsoli.",
            )
    ...
    def show_ai_gallery_in_app(self):
        ...
        if ai_gallery_index_html_path and os.path.exists(ai_gallery_index_html_path):
            ...
            self.log_message(
                f"❌ Nie znaleziono pliku galerii AI: {ai_gallery_index_html_path}",
                level="WARNING",
            )
            self.web_view.setHtml(
                "<html><body><p style='text-align:center; padding-top:50px;'>Plik galerii AI nie istnieje.</p></body></html>"
            )
